create or replace function public.reconcile_google_booking(
  p_booking_id uuid,
...
begin
  if p_starts_at <= now() or p_starts_at > now() + interval '370 days'
  ...
  message_payload := jsonb_build_object(
    'bookingId',booking.id,'startsAt',booking.starts_at,'timezone',booking.timezone
  );
  ...
  select 'call_rescheduled',booking.contact_id,contact.email,
    jsonb_build_object('startsAt',booking.starts_at,'timezone',booking.timezone,'source','google_calendar')
  from public.contacts as contact where contact.id=booking.contact_id;
  return booking;
end;
$$;

revoke all on function public.reconcile_google_booking(uuid,timestamptz,timestamptz,timestamptz)
from public,anon,authenticated;
grant execute on function public.reconcile_google_booking(uuid,timestamptz,timestamptz,timestamptz)
to service_role;
