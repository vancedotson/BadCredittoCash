...
- The calendar workspace supports month navigation, **Today**, and **Calendar / Sessions** views. Select a date to open a draft with that date prefilled, or select a saved session to view its controls and activity. The calendar groups sessions in the configured business timezone (default `America/Chicago`); the editor and selected-session details retain each session's authored timezone. New drafts start with session emails off, and read-only users can browse without edit controls.
- The session editor shows title, dates and timezone first, with a live desktop summary. **Stream & delivery** contains status, player and email controls; **Replay settings** contains recording, expiry and publication. These sections start collapsed and preserve all values while hidden. This follows [progressive disclosure](https://www.nngroup.com/articles/progressive-disclosure/): frequent edits remain on one form, while optional choices are available on demand.
- New session URL names are suggested from the title and start date. **Edit** allows a custom name that subsequent title/date changes will not overwrite. Saved session URL names remain immutable. The primary action describes the intended state: **Save draft**, **Create scheduled session**, or **Save session** for edits. Saving a draft does not automatically schedule it or enable its emails.
- The native dialog keeps keyboard navigation inside the editor and returns focus to its opener after dismissal, following the [W3C modal dialog pattern](https://www.w3.org/WAI/ARIA/apg/patterns/dialog-modal/). The action footer stays visible on small screens; validation reveals and focuses affected optional fields. Failed requests retain the draft, and pending requests prevent duplicate submissions or dismissal. Existing API validation, timezone/DST rules, permissions and site activation gates are unchanged.
