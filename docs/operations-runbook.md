...
8. Keep `LIVE_WEBINAR_ENABLED`, `CLOUDFLARE_STREAM_ENABLED`, and `LIVE_WEBINAR_EMAILS_APPROVED` off until separately authorized. When a rehearsal is authorized, an admin prepares an input through CRM Broadcast Studio; independently verify recording mode is off and that participants request playback with their personal registration link.
