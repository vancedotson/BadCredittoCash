...
const session: LiveWebinarSession = {
  ...
  status: "scheduled", embedUrl: "https://www.youtube.com/embed/live", replayUrl: "https://player.vimeo.com/video/replay",
...
describe("live webinar editor drafts", () => {
  it("uses a selected calendar date without enabling registration, emails or replay", () => {
    expect(draftOf(null, "2026-10-08", "Europe/Lisbon")).toEqual({
      ...
      status: "draft", embedUrl: "", replayUrl: "", replayPublished: false, replayAvailableUntil: "", automationEnabled: false,
    });
  ...
  it("preserves existing identity and delivery settings in the session's authored timezone", () => {
    expect(draftOf(session, "2027-01-01", "Europe/Lisbon")).toEqual({
      ...
      timezone: session.timezone, status: "scheduled", embedUrl: session.embedUrl, replayUrl: session.replayUrl,
      replayPublished: true, replayAvailableUntil: "2026-10-15T13:00", automationEnabled: true,
    });
