            ...
            return (
              <div key={i} className={`group min-h-[64px] rounded-lg border p-1 sm:min-h-[96px] sm:p-1.5 ${isToday ? "border-trust bg-sky/40" : "border-mist bg-card"}`} {...dropProps(c.key)}>
                <div className="mb-1 flex items-center justify-between">
                  <button type="button" onClick={() => setDayOpen(c.key)} aria-label={`Open ${dayHeading(c.key)}`} className={`rounded px-0.5 text-xs ${isToday ? "font-bold text-trust" : "text-slate hover:text-heading"}`}>{c.day}</button>
                  <div className="hidden items-center gap-1 sm:flex">{bk.length ? <button type="button" onClick={() => setDayOpen(c.key)} aria-label={`Open ${dayHeading(c.key)} with ${bk.length} booked call${bk.length === 1 ? "" : "s"}`} className="rounded-full bg-green/15 px-1.5 text-[10px] font-medium text-green">{bk.length} call</button> : null}<button type="button" onClick={() => setDayOpen(c.key)} aria-label={`Add task on ${dayHeading(c.key)}`} className="hidden text-slate group-hover:inline">＋</button></div>
                </div>
    ...
    <div className="fixed inset-0 z-40 grid place-items-center bg-navy/40 p-4" onClick={onClose}>
      <div role="dialog" aria-modal="true" aria-labelledby="calendar-day-title" className="w-full max-w-md rounded-2xl border border-mist bg-card p-6" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={async (e) => { e.preventDefault(); if (title.trim() && email) { try { await onAdd(dayKey, email, title.trim()); setTitle(""); } catch { /* The parent renders the recoverable error and preserves this draft. */ } } }} className="flex flex-wrap gap-2 border-t border-mist pt-3">
          <select value={email} onChange={(e) => setEmail(e.target.value)} aria-label="Contact" className="min-w-0 flex-1 rounded-lg border border-mist bg-card px-2 py-1.5 text-sm outline-none focus:border-trust">{contacts.map((c) => <option key={c.id} value={c.email}>{c.name}</option>)}</select>
          <input value={title} onChange={(e) => setTitle(e.target.value)} aria-label="Task title" placeholder="Add a task…" className="min-w-0 flex-[2] rounded-lg border border-mist bg-card px-2 py-1.5 text-sm outline-none focus:border-trust" />
