...
export function AddContactModal({ owners, onClose, onDone }: { owners: string[]; onClose: () => void; onDone: () => void }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try { await api("/api/crm/contact", "POST", v); onClose(); router.refresh(); onDone(); } catch { setErr("Could not create the contact."); setPending(false); }
  }
  return (
    <Shell title="Add contact" onClose={onClose}>
      <form onSubmit={submit} className="space-y-3">
        <input value={v.name} onChange={(e) => setV({ ...v, name: e.target.value })} placeholder="Name" className={field} autoFocus />
        ...
        <Actions pending={pending} onClose={onClose} label="Add contact" />
      </form>
    </Shell>
  );
}

export function AddTaskModal({ contacts, owners, presetEmail, onClose, onDone }: { contacts: ContactOption[]; owners: string[]; presetEmail?: string; onClose: () => void; onDone: () => void }) {
  const router = useRouter();
  const [v, setV] = useState({ email: presetEmail ?? contacts[0]?.email ?? "", title: "", type: "follow_up" as TaskType, priority: "normal" as TaskPriority, owner: "", date: "" });
  const [pending, setPending] = useState(false);
  const [err, setErr] = useState<string | null>(null);
  async function submit(e: React.FormEvent) {
    e.preventDefault();
    ...
    try {
      ...
      onClose(); router.refresh(); onDone();
    } catch { setErr("Could not create the task."); setPending(false); }
  }
  return (
    <Shell title="Add task" onClose={onClose}>
      <form onSubmit={submit} className="space-y-3">
        ...
        <input value={v.title} onChange={(e) => setV({ ...v, title: e.target.value })} placeholder="Task title" className={field} autoFocus />
        <div className="flex gap-3">
        ...
        {err ? <p className="text-sm text-red">{err}</p> : null}
        <Actions pending={pending} onClose={onClose} label="Add task" />
      </form>
    </Shell>
  );
}
