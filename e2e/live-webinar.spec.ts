...
function session(overrides: Partial<PublicLiveWebinarSession> = {}): PublicLiveWebinarSession {
  return { id: sessionA, slug: "october-live", title: "October collector rights session",
    ...
    timezone: "America/New_York", status: "scheduled", embedUrl: "https://www.youtube-nocookie.com/embed/test",
...
async function mockPublicApis(page: Page, initial: PublicLiveWebinarSession | null, linked = true) {
  ...
  await page.route("**/api/**", async (route) => {
    ...
    if (path === "/api/live/session") {
      ...
      await route.fulfill({ json: { session: current, participant: participantLinked && current ? {
  ...
  await page.route("https://www.youtube-nocookie.com/**", (route) => route.fulfill({ contentType: "text/html", body: "Mock player" }));
...
test("a cold room visit asks for the email joining link and never claims participation", async ({ page }) => {
  ...
  await page.goto(`/live/room?session=${sessionA}`);
  await expect(page.getByRole("textbox", { name: "ASK VANCE A QUESTION" })).toBeDisabled();
  await expect(page.getByText(/To send a question, open your personal joining link/)).toBeVisible();
  expect(mocks.activities).toEqual([]);
});
...
test("a missing stream records room entry without fabricating attendance", async ({ page }) => {
  ...
  await page.goto(`/live/room?session=${sessionA}`);
  await expect(page.getByText("STREAM NOT CONNECTED")).toBeVisible();
  await page.clock.fastForward(65_000);
  expect(mocks.activities.some((event) => event.event === "webinar_room_opened")).toBe(true);
  expect(mocks.activities.some((event) => event.event === "live_presence")).toBe(false);
});

test("replay availability expires while open and links retain the session", async ({ page }) => {
  ...
  await expect(page.locator('iframe[title$="— replay"]')).toBeVisible();
  await expect(page.getByRole("link", { name: "Book my free call" })).toHaveAttribute("href", `/live/call?session=${sessionA}`);
  await expect.poll(() => mocks.activities.some((event) => event.event === "live_replay_opened")).toBe(true);
  await page.clock.fastForward(31_000);
  await expect(page.getByRole("heading", { name: "The replay window has closed." })).toBeVisible();
  await expect(page.locator('iframe[title$="— replay"]')).toHaveCount(0);
  mocks.setSession(session({ id: sessionB, replayPublished: false }));
  await page.goto(`/live/replay?session=${sessionB}`);
  await expect(page.getByRole("heading", { name: "No recording is posted yet." })).toBeVisible();
  expect(mocks.activities.filter((event) => event.event === "live_replay_opened")).toHaveLength(1);
});

test("a published replay stays unavailable until the live session has ended", async ({ page }) => {
  const mocks = await mockPublicApis(page, session({ replayUrl: "https://www.youtube-nocookie.com/embed/replay",
...
test("an explicit cancelled session cannot show a registration form or a player", async ({ page }) => {
  ...
  for (const path of ["/live", "/live/room", "/live/replay", "/live/confirmed"]) {
